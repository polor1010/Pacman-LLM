...
import time, os
import traceback
from gemini_AI import GhostController
    ...
    while not self.gameOver:
      ...
      if self.catchExceptions:
        ...
        if agentIndex > 0 :
          ...
          if (counter + 1 ) % 5 == 0:
            #輸入 AI 預測的目標位置
            action, dist  = agent.getAction(observation,target = target_Pos[agentIndex])
          else:
            action, dist  = agent.getAction(observation,target = pacmanPosition)

            #self.display.updateDistributions( dist )

        else :
          print('t=',counter+1)
          #agentIndex 0 為 pacman
      ...
      if agentIndex > 0 :
        ...
        print('Ghost',agentIndex, 'Pos',gPos[agentIndex])
        ...
        if (counter + 1 ) % 5 == 0:
          ...
          if agentIndex == 2:
            target_Pos[1], target_Pos[2] = GhostController(data)
            print('AI prediction : ','Ghost 1 Pos',target_Pos[1],'Ghost 2 Pos', target_Pos[2])
