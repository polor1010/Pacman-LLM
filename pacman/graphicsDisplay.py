...
class PacmanGraphics:
  ...
  def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
    ...
    '''
    pos = self.getPosition(ghost)
    #pos = ghost[ghostIndex].target_pos 
    screen_pos = self.to_screen(pos)
    square_size = self.gridSize * 0.6  # Making square slightly smaller than cell
    square(screen_pos, 
          square_size,
          color=formatColor(0, 1, 0),  # Red color
          filled=False)
    print('moveGhost',pos)'
    '''
  ...
  def updateDistributions(self, distributions):
    ...
    for x in range(len(self.distributionImages)):
      for y in range(len(self.distributionImages[0])):
        ...
        weights = [dist[ (int(x),int(y)) ] for dist in distributions]
